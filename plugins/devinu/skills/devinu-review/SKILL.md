...
**重要:** 5 つの犬 agent + セクション 5 の pr-review-toolkit 2 agent（計 7 つ）を 1 つのメッセージ内で同時に行うことで並列実行する。
...
### 5. pr-review-toolkit 起動

セクション 4 の犬 agent と**同時に並列起動**する:
- `pr-review-toolkit:code-reviewer`
- `pr-review-toolkit:silent-failure-hunter`

pr-review-toolkit plugin が利用不可の場合（エラーが返った場合）はスキップし、犬 agent の結果のみで続行する。
...
1. セクション 3.5 で取得した既存 Sticky コメントの body から、前回の指摘一覧（file・line・タイトル・チェック状態）をパースする
2. 前回 `- [x]` だった指摘はそのまま `- [x]` を維持する（再度未チェックにしない）
3. 今回の diff を確認し、前回指摘された行が変更されている（修正コミットが含まれる）場合、その指摘を `- [x]` に更新しタイトルに取り消し線を付ける
...
各指摘をチェックボックス + blockquote 形式で表示する:

- [ ] **[{Severity}] {タイトル}** {犬アイコン列}
  `{file}:{line}`
  {説明}
  ```diff
  - {変更前}
...
- Severity は plain text で `**[Critical]**`, `**[High]**` と表記する
- 犬名はアイコンのみ並べる（統計テーブルで対応表がある）
- 修正済みの場合は `- [x]` に変更し、タイトルに取り消し線を付ける
...
|----------|------|------|--------|
| **High** | [`{file}:{line}`]({link}) | {タイトル} | `{修正案の要約}` |
| Medium | [`{file}:{line}`]({link}) | {タイトル} | — |
| Low | [`{file}:{line}`]({link}) | {タイトル} | — |
...
- **Severity 列**: Critical / High は `**{Severity}**`（太字）、Medium / Low は `{Severity}`（通常）で表記する
