...
| 変数名 | デフォルト | 説明 |
|--------|-----------|------|
| `CHOLLOWS_CI` | （未設定） | CI モード判定。`true` であれば CI モード（GHA / ScrewDriver 両対応） |
...
| `REVIEW_DRAFT` | `false` | `true` の場合、Draft PR でもレビューを実行する |
...
`CHOLLOWS_CI` が `true` で、かつ `IS_DRAFT` が `true` で、かつ `REVIEW_DRAFT` が `true` でない場合:

```
Draft PR のためレビューをスキップします。PR が Ready になったら再度実行してください。
```

を出力して終了する。ローカル実行（`CHOLLOWS_CI` 未設定）の場合はスキップしない。`REVIEW_DRAFT=true` の場合は Draft でもレビューを実行する。
...
CI モード（`CHOLLOWS_CI=true`）の場合のみ以下を実行する。
